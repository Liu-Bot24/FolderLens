...
public sealed class DetailedPropertiesTests
{
    ...
    public async Task RealSonyArwExifComesFromReadOnlyWorkerProbe()
    {
        ...
        await catalog.Write(c=>{using var cmd=c.CreateCommand();cmd.CommandText="UPDATE Files SET name=$name,relative_path=$name,logical_bytes=$length,mtime_utc_ticks=$modified";cmd.Parameters.AddWithValue("$name",Path.GetFileName(input));cmd.Parameters.AddWithValue("$length",length);cmd.Parameters.AddWithValue("$modified",modified);return cmd.ExecuteNonQuery();});
