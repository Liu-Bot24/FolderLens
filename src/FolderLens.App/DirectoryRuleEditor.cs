...
internal sealed class DirectoryRuleEditor
{
    ...
    internal DirectoryRuleEditor(DirectoryRule[] initial,Func<Task<string?>> pick,Func<DirectoryRule[],CancellationToken,Task<DirectoryRulePreview>> preview,CancellationToken cancellation)
    {
        ...
        remove.Click+=(_,_)=>{if(list.SelectedIndex is var i&&i>=0){rules.RemoveAt(i);Refresh();}};View.Children.Add(remove);
        ...
        var add=new Button{Content="添加规则"};add.Click+=(_,_)=>
        {
            try
            {
                ...
                if(!rules.Contains(rule))rules.Add(rule);pattern.Text="";Refresh();
            }
            catch(Exception ex){feedback.Text=ex.Message;}
        };View.Children.Add(add);
    ...
    private void Refresh()
    {
        ...
        foreach(var rule in rules.ToArray())
        {
            ...
            box.Checked+=(_,_)=>{rules[index]=rules[index] with{Enabled=true};feedback.Text="";};
            box.Unchecked+=(_,_)=>{rules[index]=rules[index] with{Enabled=false};feedback.Text="";};list.Items.Add(box);
        }
    }
