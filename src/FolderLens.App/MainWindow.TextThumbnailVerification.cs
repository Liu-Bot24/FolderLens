...
public sealed partial class MainWindow
{
    private async Task VerifyTextThumbnails(string source,Dictionary<string,object> report)
    {
        ...
        try
        {
            ...
            foreach(int size in new[]{160,240})
            {
            ...
            foreach(int size in new[]{160,100,240}){ThumbnailSize.Value=size;UpdateGridPresentation();await Task.Delay(250);}
            ...
            if(reads.Any(bytes=>bytes>1024))throw new InvalidOperationException("摘录超出读取预算。");
