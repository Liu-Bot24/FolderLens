...
public sealed partial class MainWindow
{
    ...
    private async Task OperateFile(FileOperationKind kind)
    {
        ...
        try
        {
            ...
            {
                await new ContentDialog{XamlRoot=Shell.XamlRoot,Title="请选择一个文件",Content="重命名、移动和删除目前每次操作一个文件。",CloseButtonText="关闭"}.ShowAsync();return;
            }
            ...
            if(closing||rootVersion!=rootChangeVersion||!ReferenceEquals(selected,row))return;
            await ExternalFileLaunch.Resolve(store,prefetchSourceProbe,SourceRootPath(row),SourceRootId(row),item.EntryId,item.Version,false,lifetime.Token);
            var stamp=Stamp(row)??throw new IOException("请等待文件信息加载后重试。");
            // Release our own preview/decoder handles before asking Windows to move it.
            await ReturnToBrowser();ClearResultSelection();CancelThumbnails();ClearPrefetchedImages();ClearImage();
            Status.Text=kind==FileOperationKind.Recycle?"正在删除文件…":"正在更新文件…";
            await FileOperations.Execute(new(source,stamp,kind,destination),lifetime.Token);
            await store.RetireOperatedFile(item,lifetime.Token);
        ...
        catch(OperationCanceledException){if(!closing){Status.Text="文件操作已取消。";await RefreshQuery();}}
        catch(Exception error){RecordWebView($"FileOperation failed kind={kind} entry={item.EntryId} error={error}");ShowError(error);if(!closing)await RefreshQuery();}
