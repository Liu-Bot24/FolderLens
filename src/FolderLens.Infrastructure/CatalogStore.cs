...
public sealed record SnapshotItem(long Ordinal,string EntryId,long Version,string RelativePath,string DirectoryId,long Bytes,long? Allocated,string Kind)
{
    ...
    public string? DirectoryLocationId {get;init;}
...
public sealed partial class CatalogStore : IAsyncDisposable
{
    ...
    private static bool InitializeSchema(SqliteConnection c,string name,CancellationToken cancellation,Action<string>? progress)
    {
        ...
        if(name=="sessions"&&version==4)
        {
            ...
            cmd.CommandText="ALTER TABLE ResultItems ADD COLUMN observed_directory_location_id TEXT;ALTER TABLE ResultItems ADD COLUMN observed_binding_revision INTEGER NOT NULL DEFAULT 0;PRAGMA user_version=5;";
    ...
    public Task<FirstResultPage> ReadFirstPage(FilterSpec filter,CancellationToken cancellation=default)=>interactiveReader.Execute(c=>
    {
        ...
        cmd.CommandText=$"SELECT f.entry_id,f.file_version,f.relative_path,f.directory_id,f.logical_bytes,f.allocated_bytes,f.kind,f.root_id,r.display_path,r.root_epoch,f.path_revision,b.location_id,b.binding_revision,f.physical_identity FROM Files f JOIN Roots r ON r.root_id=f.root_id JOIN DirectoryLocationBindings b ON b.directory_id=f.directory_id {(grouping is null?"":FolderGroupingQuery.Join)} WHERE {predicate} ORDER BY {(grouping is null?"":FolderGroupingQuery.OrderPrefix)}{query.OrderBy} LIMIT 256";
        ...
        while(rows.Read()){cancellation.ThrowIfCancellationRequested();items.Add(new(items.Count,rows.GetString(0),rows.GetInt64(1),rows.GetString(2),rows.GetString(3),rows.GetInt64(4),rows.IsDBNull(5)?null:rows.GetInt64(5),rows.GetString(6)){SourceRootId=rows.GetString(7),SourceRootPath=rows.GetString(8),SourceRootEpoch=rows.GetInt64(9),PathRevision=rows.GetInt64(10),DirectoryLocationId=rows.GetString(11),BindingRevision=rows.GetInt64(12),PhysicalIdentity=rows.IsDBNull(13)?null:rows.GetString(13)});}
    ...
    private ResultHandle BuildSnapshot(SqliteConnection c,FilterSpec filter,long epoch,long generation,CancellationToken cancellation)
    {
        ...
        try
        {
            ...
            read.CommandText=$"SELECT f.entry_id,f.file_version,f.path_revision,f.directory_id,f.relative_path,f.logical_bytes,f.allocated_bytes,f.physical_identity,f.kind,{query.StateExpression} AS match_state,{groupColumns},f.root_id,r.display_path,r.root_epoch,b.location_id,b.binding_revision FROM Files f JOIN Roots r ON r.root_id=f.root_id JOIN DirectoryLocationBindings b ON b.directory_id=f.directory_id {(grouping is null?"":FolderGroupingQuery.Join)} WHERE {query.CandidateExpression} ORDER BY {(grouping is null?"":FolderGroupingQuery.OrderPrefix)}{query.OrderBy}";
            foreach(var p in query.Parameters)read.Parameters.AddWithValue(p.Key,p.Value);
            using var source=read.ExecuteReader();
            using var add=session.CreateCommand();add.CommandText="INSERT INTO ResultItems(session_id,ordinal,entry_id,observed_version,observed_path_revision,directory_id,snapshot_relative_path,snapshot_logical_bytes,snapshot_allocated_bytes,snapshot_physical_identity,snapshot_kind,group_id,source_root_id,source_root_path,source_root_epoch,observed_directory_location_id,observed_binding_revision) VALUES($id,$ordinal,$entry,$version,$pathrev,$dir,$path,$bytes,$allocated,$physical,$kind,$group,$sourceId,$sourcePath,$sourceEpoch,$location,$binding)";
            foreach(string key in new[]{"$id","$ordinal","$entry","$version","$pathrev","$dir","$path","$bytes","$allocated","$physical","$kind","$group","$sourceId","$sourcePath","$sourceEpoch","$location","$binding"})add.Parameters.Add(new SqliteParameter(key,DBNull.Value));
            ...
            try
            {
                while(source.Read())
                {
                    ...
                    add.Parameters[11].Value=source.GetValue(10);for(int i=0;i<5;i++)add.Parameters[12+i].Value=source.GetValue(14+i);
    ...
    public Task<IReadOnlyList<SnapshotItem>> ReadPage(string sessionId,long ordinal,int count=256,CancellationToken cancellation=default)
    {
        ...
        return sessionReader.Execute<IReadOnlyList<SnapshotItem>>(c=>
        {
            ...
            while(rows.Read())result.Add(new(rows.GetInt64(0),rows.GetString(1),rows.GetInt64(2),rows.GetString(3),rows.GetString(4),rows.GetInt64(5),rows.IsDBNull(6)?null:rows.GetInt64(6),rows.GetString(7)){Group=rows.IsDBNull(8)?null:ReadGroup(rows,8),SourceRootId=rows.IsDBNull(16)?null:rows.GetString(16),SourceRootPath=rows.IsDBNull(17)?null:rows.GetString(17),SourceRootEpoch=rows.IsDBNull(18)?null:rows.GetInt64(18),PathRevision=rows.GetInt64(19),DirectoryLocationId=rows.IsDBNull(20)?null:rows.GetString(20),BindingRevision=rows.GetInt64(21),PhysicalIdentity=rows.IsDBNull(22)?null:rows.GetString(22)});
            return result;
        },cancellation);
    }
