...
public sealed partial class CatalogStore
{
    public Task<ExtensionChoices> ReadFileExtensions(FilterSpec scope,CancellationToken cancellation=default)=>interactiveReader.Execute(c=>
    {
        ...
        var filter=new FilterSpec{RootId=scope.RootId,DirectoryScope=scope.DirectoryScope,ScopeDirectFiles=scope.ScopeDirectFiles,
            Recursive=scope.Recursive,Kinds=scope.Kinds,Extensions=scope.Extensions,ShowHidden=scope.ShowHidden,
