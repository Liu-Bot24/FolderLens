...
public sealed class CapacityService(CatalogStore catalog)
{
    ...
    {
        using var transaction=c.BeginTransaction(deferred:true);using var state=c.CreateCommand();state.Transaction=transaction;state.CommandText="SELECT scan_state FROM Roots WHERE root_id=$root";state.Parameters.AddWithValue("$root",root);string status=state.ExecuteScalar()?.ToString()??throw new InvalidOperationException("该根目录没有可用的索引统计。");
        ...
        var result=Capacity.Build(Files(),paths,cancellation);transaction.Commit();return new CapacityReport(root,status,result);
    },cancellation);
