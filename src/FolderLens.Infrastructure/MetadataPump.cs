...
public sealed class MetadataPump(CatalogStore catalog,WorkerClient worker,MediaTools? media=null,string? scanExecutable=null)
{
    ...
    public Task FillAll(string rootId,string root,long epoch,IProgress<long>? progress,CancellationToken cancellation)=>Fill(rootId,root,epoch,progress,true,cancellation);
    private async Task Fill(string rootId,string root,long epoch,IProgress<long>? progress,bool includeMedia,CancellationToken cancellation)
    {
            ...
            while(true)
            {
                ...
                var batch=await catalog.Read(c=>
                {
                    using var cmd=c.CreateCommand();cmd.CommandText="""
                        SELECT entry_id,relative_path,file_version,kind,logical_bytes,mtime_utc_ticks
                        FROM Files f WHERE root_id=$root AND entry_id>$after AND entry_state='present' AND hydration_state='local'
                    ...
                    using var rows=cmd.ExecuteReader();var entries=new List<(string Id,string Path,long Version,string Kind,long Length,long Modified)>();
                    while(rows.Read())entries.Add((rows.GetString(0),rows.GetString(1),rows.GetInt64(2),rows.GetString(3),rows.GetInt64(4),rows.GetInt64(5)));return entries;
                ...
                foreach(var entry in batch)observations.Add(entry.Id,await Observe(sourceProbe,root,entry.Path,cancellation).ConfigureAwait(false));
                foreach(var entry in batch)
                {
                    ...
                    try
                    {
                        ...
                        if(entry.Kind=="image")
                        {
                            var reply=await worker.Request(path,"probe",new(rootId,epoch,1,1,entry.Version,1),new(),cancellation,before).ConfigureAwait(false);
                            ...
                            if(await sourceProbe.Read(path,cancellation).ConfigureAwait(false)!=before)throw new IOException("FileChanged");
                            await catalog.ApplyImageMetadata(entry.Id,entry.Version,rootId,epoch,metadata.GetProperty("width").GetInt32(),metadata.GetProperty("height").GetInt32(),metadata.GetProperty("format").GetString()!,metadata.GetProperty("isRaw").GetBoolean(),metadata.GetProperty("isAnimated").GetBoolean(),metadata.GetProperty("provider").GetString()!,cancellation).ConfigureAwait(false);
                            ...
                            await catalog.ApplyFileDetails(entry.Id,entry.Version,rootId,epoch,details,metadata.GetProperty("provider").GetString()!,cancellation).ConfigureAwait(false);
                        }
                        else
                        {
                            var metadata=await media!.Probe(path,cancellation).ConfigureAwait(false);
                            if(await sourceProbe.Read(path,cancellation).ConfigureAwait(false)!=before)throw new IOException("FileChanged");
                            await catalog.ApplyMediaMetadata(entry.Id,entry.Version,rootId,epoch,metadata,"ffprobe-v1",cancellation).ConfigureAwait(false);
                            if(metadata.Details is {} details)await catalog.ApplyFileDetails(entry.Id,entry.Version,rootId,epoch,details,"ffprobe-v1",cancellation).ConfigureAwait(false);
                        }
                    }
                    ...
                    catch(Exception ex) when(ex is not OperationCanceledException)
                    {
                        ...
                        await catalog.MarkMetadataFailure(entry.Id,entry.Version,rootId,epoch,entry.Kind=="image"?["identity","imageGeometry","animation","captureTime","imageColor"]:["identity","media","imageGeometry","captureTime"],error,error=="UnsupportedCodec",cancellation).ConfigureAwait(false);
                    }
