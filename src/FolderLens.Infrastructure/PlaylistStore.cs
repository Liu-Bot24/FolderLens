...
public sealed partial class CatalogStore
{
    ...
    private static int InitializePlaylist(SqliteConnection c,string path)
    {
        ...
        Execute(c,"""
            ...
            """);
        if(Scalar(c,"SELECT version FROM playlist.PlaylistInfo")!=1)throw new InvalidDataException("收藏库由较新版本创建，请使用匹配版本。");
        ...
        return Execute(c,"""
            ...
            CREATE TEMP TRIGGER Playlist_Add AFTER INSERT ON main.CollectionMembers BEGIN
                INSERT OR IGNORE INTO SavedLinks
                ...
                FROM Files f JOIN Roots r ON r.root_id=f.root_id JOIN DirectoryLocations l ON l.location_id=NEW.directory_location_id WHERE f.entry_id=NEW.entry_id;
            END;
    ...
    public Task ImportLegacyPlaylists(string legacyPath,CancellationToken cancellation=default)=>writer.Execute(c=>
    {
        ...
        if(File.Exists(legacyPath))
        {
            ...
            read.CommandText="""
                SELECT m.collection_id,coalesce(l.anchor_locator,''),coalesce(l.directory_identity,''),m.location_key,r.display_path,f.relative_path,m.added_utc_ticks
            ...
            {
                cancellation.ThrowIfCancellationRequested();
                DirectoryIndexer.Execute(c,transaction,"INSERT OR IGNORE INTO playlist.SavedLinks VALUES($id,$anchor,$identity,$key,$path,$time)",
                    ("$id",rows.GetString(0)),("$anchor",rows.GetString(1)),("$identity",rows.GetString(2)),("$key",rows.GetString(3)),("$path",Path.Combine(rows.GetString(4),rows.GetString(5))),("$time",rows.GetInt64(6)));
            }
            stable.Commit();
        }
    ...
    public async Task RefreshPlaylist(string collectionId,CancellationToken cancellation=default)
    {
        ...
        while(true)
        {
            ...
            {
                using var cmd=c.CreateCommand();cmd.CommandText="SELECT rowid,path,anchor,directory_identity,location_key FROM playlist.SavedLinks WHERE collection_id=$id AND rowid>$after ORDER BY rowid LIMIT 64";
                cmd.Parameters.AddWithValue("$id",collectionId);cmd.Parameters.AddWithValue("$after",offset);
                using var rows=cmd.ExecuteReader();var batch=new List<(long Id,string Path,string Anchor,string Identity,string Key)>();
                while(rows.Read())batch.Add((rows.GetInt64(0),rows.GetString(1),rows.GetString(2),rows.GetString(3),rows.GetString(4)));return batch;
            ...
            foreach(var link in links)
            {
                ...
                async Task<ScanDirectoryPacket> Probe(string value)=>worker is null?await Task.Run(()=>ScanPathProbe.Read(value),cancellation).ConfigureAwait(false):await worker.Probe(value,cancellation).ConfigureAwait(false);
                ...
                bool changed=directory.PhysicalIdentity!=link.Identity||directory.ResolvedLocation!=link.Anchor||file.State=="missing"||file.State=="present"&&file.PhysicalIdentity is not null&&key!=link.Key;
                if(changed)
                {
                    await RemoveSavedLink(collectionId,link.Id,link.Anchor,link.Identity,link.Key,cancellation).ConfigureAwait(false);
                    continue;
                }
                if(file.State!="present"||file.FileStamp is null||file.PhysicalIdentity is null||key!=link.Key)continue;
