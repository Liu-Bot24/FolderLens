...
卸载默认保留用户设置与索引，提供明确的“同时删除应用数据”选项；删除只限应用自有真实目录，检查重解析点。不能触碰最近访问的素材根。更新数据库使用事务迁移，失败回滚，默认不复制整库；成功升级不支持直接降级，详见[事务升级决策](adr/2026-09-14-transactional-startup-upgrade.md)。较旧程序遇到较新schema明确提示，不自动破坏性重建用户预设。

便携包与安装包只收录通过路径、隐私和哈希校验的发布清单条目。安装包使用生成的逐文件清单，不递归收录运行过的程序目录；安装模式不携带 `portable.json`。该规则不替代安装及运行验收。
