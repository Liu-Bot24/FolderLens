...
所有权链：AppSession 释放根时取消根任务；BrowserQuerySession 释放时取消查询和列表预取；PreviewSession 释放时取消图片/文本/音频；SurfaceLease 释放 CPU/GPU/共享内存资源；worker 退出立即使其所有 lease 失效。资源不能靠最终 GC 才释放。

### 版本标识

| 标识 | 变更条件 | 作用 |
|---|---|---|
| rootEpoch | 切根/关闭根/重建当前根上下文 | 拒绝旧根任务回写 |
