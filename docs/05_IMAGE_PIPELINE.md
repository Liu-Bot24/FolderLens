...
| TIFF/TIF | MUST 单页静态；多页按普通文件 | 单页 8/16位、常见 LZW/ZIP；多页显示文件信息并外部打开；BigTIFF 有真实样本才列 supported |
...
动画暂停、切换文件、最小化且不可见时停止帧调度；关闭释放worker会话。损坏尾帧显示当前已解码内容+错误，不冻结主UI。多页 TIFF 检测后按普通文件处理，不提供图片逐页浏览；单页图像和时间动画不受此规则影响。
