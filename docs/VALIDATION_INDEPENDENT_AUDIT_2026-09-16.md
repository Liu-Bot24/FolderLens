...
后续尝试独立命名的 WPR HeapSnapshot 跟踪，Windows 返回 `0x80070005 / Access is denied`，没有取得分配栈，也未启动新的测试进程或修改 IFEO。权限失败不能视为内存问题已解决。

源码修复提交 `f647663` 已推送 main，原启动入口已更新。`current-entry-independent-f647663` 下的部署和最终哈希记录均 PASS：7 项部署启动检查、媒体能力探测通过，926 个用户数据文件清单未变，1,498 个运行文件哈希匹配，App DLL 与上述整批回归一致。构建标识为 `0.1.0-development-a8d0035ed224`；运行中的旧实例没有被操作，也不会自动切换到新构建。
